...
function generarAnalisis(
  ...
  if (totalEst === 0 && totalReal === 0) {
    container.innerHTML =
      '<p style="color: var(--text-muted); font-style: italic;">Ingrese los datos del producto y los costos para generar el análisis automático.</p>';
    return;
  }
  ...
  if (variacion < 0) {
    ...
    colorVar = "var(--yellow)";
    explicacion =
      "Los costos reales coinciden exactamente con los costos estándar presupuestados. Esto refleja una planificación precisa del proceso productivo.";
  }
  ...
  const desviaciones = [
    {
      ...
      est: mpEst,
      real: mpReal,
      diff: Math.abs(mpReal - mpEst),
    },
    {
      nombre: "Mano de Obra",
      est: moEst,
      real: moReal,
      diff: Math.abs(moReal - moEst),
    },
    {
      nombre: "Costos Indirectos",
      est: ciEst,
      real: ciReal,
      diff: Math.abs(ciReal - ciEst),
    },
  ];
