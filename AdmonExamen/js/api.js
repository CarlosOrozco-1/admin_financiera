...
const FinovaAPI = (function () {
    ...
    async function request(method, path, body = null, auth = true) {
        ...
        try {
            ...
            return { ok: false, status: 0, data: null, message: 'Error de conexion: ' + err.message };
        }
    }

    async function get(path, auth = true) { return request('GET', path, null, auth); }
    async function post(path, body, auth = true) { return request('POST', path, body, auth); }
    async function put(path, body, auth = true) { return request('PUT', path, body, auth); }
    async function del(path, auth = true) { return request('DELETE', path, null, auth); }
    ...
    const inventario = {
        ...
        async getStats() {
            ...
            return res.ok ? res.data : { totalItems: 0, totalValue: 0, lowStock: 0, totalUnits: 0 };
        },
        async getCategories() {
