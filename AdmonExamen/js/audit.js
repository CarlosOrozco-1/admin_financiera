...
const FiNovaAudit = (function () {
  ...
  async function log(modulo, accion, detalle) {
    ...
    try {
      ...
      var username =
        typeof FiNovaAuth !== "undefined"
          ? FiNovaAuth.getCurrentUsername()
          : "sistema";
  ...
  async function getFiltered(filters) {
    if (useAPI) {
      ...
      if (filters.fechaDesde) {
        ...
        data = data.filter(function (r) {
          return new Date(r.fecha).getTime() >= desde;
        });
      }
      if (filters.fechaHasta) {
        ...
        data = data.filter(function (r) {
          return new Date(r.fecha).getTime() <= hastaTime;
        });
      }
    ...
    if (filters.modulo && filters.modulo !== "Todos") {
      records = records.filter(function (r) {
        return r.modulo === filters.modulo;
      });
    }
    if (filters.usuario && filters.usuario !== "Todos") {
      records = records.filter(function (r) {
        return r.usuario === filters.usuario;
      });
    }
    if (filters.fechaDesde) {
      records = records.filter(function (r) {
        return r.fecha >= filters.fechaDesde;
      });
    }
    if (filters.fechaHasta) {
